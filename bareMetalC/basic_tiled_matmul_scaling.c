...
int counter_val_end[66];
  ...
  for (int i = 0; i < 66; ++i) {
	...
	printf("SCALING_DIM: %d\n\n", SCALING_DIM);
	gemmini_flush(0);

	counter_reset();

	elem_t A[SCALING_DIM][SCALING_DIM];
        elem_t B[SCALING_DIM][SCALING_DIM];
        elem_t D[SCALING_DIM][SCALING_DIM];

        acc_t C[SCALING_DIM][SCALING_DIM];

        for (size_t i = 0; i < SCALING_DIM; ++i) {
          for (size_t j = 0; j < SCALING_DIM; ++j) {
            A[i][j] = (rand() % 4) * 16;
            B[i][j] = (rand() % 4) * 16;
            D[i][j] = (rand() % 4) * 16;
          }
...
//			D[i][j] = D[i][j] * 2;
	...
	//printf("Here A");
	tiled_matmul_auto(SCALING_DIM, SCALING_DIM, SCALING_DIM,
            (elem_t*)A, (elem_t*)B,
	    &D[0][0], (elem_t*)C,
            SCALING_DIM, SCALING_DIM, SCALING_DIM, SCALING_DIM,
	...
	*/
	...
	printf("STATS\n\nCycles taken: %u\n", end-start);
    const int total_macs = SCALING_DIM * SCALING_DIM * SCALING_DIM;
	...
	for (int i = 0; i < 66; ++i) {
		printf("DIM %d: %s, %d\n", SCALING_DIM, counter_names[i], counter_val_end[i] - counter_val_start[i]);
	}
