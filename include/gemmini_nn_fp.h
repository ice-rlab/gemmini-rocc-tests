#ifndef GEMMINI_NN_FP_H
#define GEMMINI_NN_FP_H
...
struct FP_ConvParams {
    ...
    int n_patches;
    int patch_size;
    fp_acc_scale_t output_scale;
    fp_scale_t res_scale;
...
// factors
static void fp_tiled_matmul_nn(size_t dim_I, size_t dim_J, size_t dim_K,
        const fp_elem_t A[dim_I][dim_K], const fp_elem_t B[dim_K][dim_J],
        const void * D, fp_elem_t C[dim_I][dim_J],
        int act, fp_acc_scale_t scale, bool repeating_bias,
        ...
        bool check, char * layer_name)
{
    ...
    fp_tiled_matmul(dim_I, dim_J, dim_K,
        (fp_elem_t*)A, (fp_elem_t*)B, D, (fp_elem_t*)C, 
        dim_K, dim_J, dim_J, dim_J,
        FP_MVIN_SCALE_IDENTITY, FP_MVIN_SCALE_IDENTITY, FP_MVIN_SCALE_IDENTITY,
    ...
    if (check) {
        printf("%s: CPU\n", layer_name);
        fp_elem_t gold[dim_I][dim_J];
        fp_tiled_matmul_auto(dim_I, dim_J, dim_K,
            (fp_elem_t*)A, (fp_elem_t*)B, D, (fp_elem_t*)gold, 
            dim_K, dim_J, dim_J, dim_J,
            FP_MVIN_SCALE_IDENTITY, FP_MVIN_SCALE_IDENTITY, FP_MVIN_SCALE_IDENTITY,
            act, scale, 0, repeating_bias,
            false, false,
...
static void fp_tiled_matmul_nn_auto(size_t dim_I, size_t dim_J, size_t dim_K, 
        const fp_elem_t A[dim_I][dim_K], const fp_elem_t B[dim_K][dim_J],
        const void * D, fp_elem_t C[dim_I][dim_J],
        int act, fp_acc_scale_t scale, bool repeating_bias,
        enum tiled_matmul_type_t tiled_matmul_type,
...
// auto tiling calc
static void fp_tiled_matmul_nn_stride_auto(size_t dim_I, size_t dim_J, size_t dim_K,
        const size_t A_stride, const size_t B_stride, const size_t C_stride,
        const fp_elem_t * A, const fp_elem_t * B, const void * D, const fp_elem_t * C,
        int act, fp_acc_scale_t scale, bool repeating_bias,
        enum tiled_matmul_type_t tiled_matmul_type)
{
...
static void fp_conv_dw(size_t I, size_t J,
    ...
    const fp_elem_t input[batch_size][in_row_dim][in_col_dim][channels],
    const fp_elem_t weight[channels][kernel_size][kernel_size],
    const fp_acc_t * bias,
    // fp_elem_t output [batch_size][out_row_dim][out_col_dim][channels],
    fp_elem_t output [I][J],
    const struct FP_ConvParams * params)
{
        ...
        for (int channel = 0; channel < channels; channel++) {
            for (int out_row = 0; out_row < out_row_dim; out_row++) {
                for (int out_col = 0; out_col < out_col_dim; out_col++) {
                    ...
                    fp_acc_t scaled = FP_ACC_SCALE(result, params->output_scale);

                    if (scaled > fp_elem_t_max) {
                        scaled = fp_elem_t_max;
                    } else if (scaled < fp_elem_t_min) {
                        scaled = fp_elem_t_min;
                    }
...
static void fp_conv_dw_with_col2im(size_t prev_I, size_t prev_J, size_t I, size_t J,
    const size_t batch_size, const size_t channels,
    const size_t out_row_dim, const size_t out_col_dim, const size_t kernel_size,
    const fp_elem_t input[prev_I][prev_J],
    const fp_elem_t weight[channels][kernel_size][kernel_size],
    const fp_acc_t * bias,
    // fp_elem_t output [batch_size][out_dim][out_dim][channels],
    fp_elem_t output [I][J],
    const struct FP_ConvParams * params)
{
    for (int batch = 0; batch < batch_size; batch++) {
...
static void fp_im2col(size_t batch_size, size_t channels, size_t im_row_dim, size_t im_col_dim,
    size_t I, size_t K,
    const fp_elem_t input[batch_size][im_row_dim][im_col_dim][channels],
    fp_elem_t output[I][K],
    const struct FP_ConvParams * params)
{
    int patch_row = 0;
...
static void fp_im2col_with_col2im(size_t prev_I, size_t prev_J,
    size_t next_I, size_t next_K,
    const fp_elem_t input[prev_I][prev_J],
    fp_elem_t output[next_I][next_K],
    const struct FP_ConvParams * params)
{
    int out_row = 0;
...
void fp_vecadd(size_t len, const fp_elem_t * A, const fp_elem_t * B, fp_elem_t * C, fp_scale_t A_shift) {
    for (size_t i = 0; i < len; i++) {
        fp_acc_t result = FP_MVIN_SCALE(A[i], A_shift) + B[i];

        if (result > fp_elem_t_max) {
            result = fp_elem_t_max;
        } else if (result < fp_elem_t_min) {
            result = fp_elem_t_min;
        }
...
void fp_resadd1(const size_t batch_size, const size_t channels, const size_t im_dim,
    const fp_elem_t A[batch_size][im_dim][im_dim][channels],
    const fp_elem_t B[batch_size][im_dim][im_dim][channels],
    fp_elem_t C[batch_size][im_dim][im_dim][channels],
    bool relu,
    const struct FP_ConvParams * params) {
        ...
        for (size_t row = 0; row < params->out_dim_pooled; row++) {
            for (size_t col = 0; col < params->out_dim_pooled; col++) {
                for (size_t channel = 0; channel < params->out_channels; channel++) {
                    fp_acc_t result = FP_MVIN_SCALE(A[batch][row][col][channel], params->res_scale) + B[batch][row][col][channel];
...
void fp_resadd2(const size_t I, const size_t J,
    const size_t batch_size, const size_t channels, const size_t im_dim,
    const fp_elem_t A[I][J],
    const fp_elem_t B[batch_size][im_dim][im_dim][channels],
    fp_elem_t C[batch_size][im_dim][im_dim][channels],
    bool relu,
    const struct FP_ConvParams * params) {
        ...
        for (size_t row = 0; row < params->out_dim_pooled; row++) {
            for (size_t col = 0; col < params->out_dim_pooled; col++) {
                for (size_t channel = 0; channel < params->out_channels; channel++) {
                    ...
                    fp_acc_t result = FP_MVIN_SCALE(A[r][channel], params->res_scale) + B[batch][row][col][channel];
...
void fp_resadd3(const size_t I, const size_t J,
    const fp_elem_t A[I][J],
    const fp_elem_t B[I][J],
    fp_elem_t C[I][J],
    bool relu,
    const struct FP_ConvParams * params) {
        ...
        for (size_t row = 0; row < params->out_dim_pooled; row++) {
            for (size_t col = 0; col < params->out_dim_pooled; col++) {
                for (size_t channel = 0; channel < params->out_channels; channel++) {
                    ...
                    fp_acc_t result = FP_MVIN_SCALE(A[r][channel], params->res_scale) + B[r][channel];
...
// pooling
void fp_pool(size_t batch_size, size_t channels, size_t in_row_dim, size_t in_col_dim,
    size_t out_row_dim, size_t out_col_dim,
    fp_elem_t input[batch_size][in_row_dim][in_col_dim][channels],
    fp_elem_t output[batch_size][out_row_dim][out_col_dim][channels],
    const struct FP_ConvParams * params)
{
        ...
        for (int channel = 0; channel < channels; channel++) {
            for (int out_row = 0; out_row < out_row_dim; out_row++) {
                for (int out_col = 0; out_col < out_col_dim; out_col++) {
                    ...
                    fp_elem_t result = fp_elem_t_min;
...
void fp_pool_with_col2im(size_t I, size_t J,
